...
def get_data(num_samples=1, plugins=None, plugin_instances=None):
    multi_plugins = ["linux", "redisdb", "mysql", "jvm", "tpcc", "tomcat","postgres", "oozie","yarn","namenode","kafkatopic", "kafkajmx", "zookeeperjmx", "apache", "topstats", "nginx", "nginxplus", "cassandra", "hxconnect", "esalogstore", "spark"]
    if plugin_instances is None:
        plugin_instances = [ALL]
    if plugins is None:
        plugins = [ALL]
